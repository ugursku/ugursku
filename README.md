...
Your suite may not be watching, though. Express 4 → 5 is one of the best-known breaking changes in JavaScript — routing and parameter handling moved, and the ecosystem wrote migration guides for it. I installed Express 5 into four projects that depend on it — `cors`, `multer`, `express-session`, `formidable` — and ran each project's own tests. [Not one of them went red.](https://github.com/patchery-dev/Patchery/actions/runs/34102236170) That is a limit of the trigger rather than a claim about the fix: Patchery starts when your build goes red, so a break your suite never sees is one it never sees either. Where the suite *does* go red it is watching that path, which is what makes turning it green again worth something. (A fifth project was already failing before the upgrade, so it could not be measured.)
...
Patchery™ is a GitHub Action you install yourself — there is no hosted service and Patchery stores nothing. What leaves your runner goes only to the endpoint you configure: the failing test output, the files the agent opens, and the diff it writes. Source-available under BUSL-1.1, free for public repositories; licence covers code not name. Private commercial use is a subscription — ugur@patchery.dev. Working in JavaScript, expanding from there.
