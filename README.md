...
Repo	What changed
ianarawjo/ChainForge#416	OpenAI SDK v3 → v4: three call sites, response unwrapping, APIError handling
ToolJet/ToolJet#17829	Gemini plugin moved off @google/generative-ai, end-of-life since 30 Nov 2025
Caknoooo/chatgpt3-openai-api#2	openai v3 calling text-davinci-003, shut down in January 2024; also removed an API key being logged to the console
Patchery™ is a GitHub Action you install yourself — there is no hosted service and nothing of yours is stored anywhere. What leaves your runner is what a model has to read: the failing test output, the files the agent opens, and the diff it writes, sent only to the endpoint you configure. Source available under BUSL-1.1, free for public repositories; the licence covers the code and not the name. Working in JavaScript, expanding from there.
